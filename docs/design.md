...
ESP32-2432S028Rが、Codexの5時間制限と週間制限を自律的に表示し、Wi-Fi経由で約5分ごとに更新する。初期設定とブラウザデバイス認証だけはPCまたはスマートフォンを使うが、その後の常時接続は要求しない。初期設定APは15分で自動停止し、接続タブから再開できる。
...
画面は使用量、設定、接続の3タブで構成する。使用量タブは `Snapshot` の5時間・週間ウィンドウを表示する。各ウィンドウのバーの下には `reset_at` をJSTへ換算したリセット日時と残り時間を表示する。残り時間は基本「あとX日Y時間」、24時間未満だけ「あとX時間Y分」（1時間未満は分だけ）とし、取得がなくても分の変わり目で再描画する。設定タブは0〜59分のスライダー、0〜24時間のスライダー、60〜600秒（60秒刻み）の取得間隔スライダーとスクロールバーを備える。0分0時間は自動消灯オフ（常時点灯）と同じである。接続タブはWi-Fi状態、AP情報、デバイスコード、IPアドレスを表示する。
...
使用量エンドポイントは公開API契約ではなく、公式ログイン済みクライアントが使うprivate endpointである。レスポンス形式、認証方式、利用可否は変更され得るため、失敗を空データとして隠さず、再ログインや後で再試行できる状態にする。リセット日時は `primary_window`・`secondary_window` の `reset_at`（秒単位UNIX時刻、UTC）を使い、公式 codex-rs の backend-client（`map_rate_limit_window` が `reset_at` を `resetsAt` へ写像）と同義とする。`reset_after_seconds` は表示に使わない。公式料金案内も現在の上限とリセット時刻は使用量ダッシュボードで確認する方式である。CDP実測では未認証の `GET /wham/usage` が `{"detail":"Unauthorized"}` を返し、Bearer認証が必須であることを確認した。
...
- 初期設定HTTP画面（日本語・英語）。
- デバイス認証の保留・完了・期限切れ。
- 5分周期の使用量取得と前回値保持。取得失敗時は2倍ずつ延ばし（最大15分）、成功で5分へ戻す。手動更新とWi-Fi再接続時は即時取得する。
