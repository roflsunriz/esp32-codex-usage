...
画面は使用量、設定、接続の3タブで構成する。使用量タブは `Snapshot` の5時間・週間ウィンドウを表示する。各ウィンドウのバーの下には `reset_at` をJSTへ換算したリセット日時と残り時間を表示する。残り時間は基本「あとX日Y時間」、24時間未満だけ「あとX時間Y分」（1時間未満は分だけ）とし、取得がなくても分の変わり目で再描画する。設定タブは15秒、30秒、1分、2分、5分、10分、30分、1時間、2時間を許可する。接続タブはWi-Fi状態、AP情報、デバイスコード、IPアドレスを表示する。
