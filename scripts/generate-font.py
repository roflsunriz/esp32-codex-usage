...
def main():
    ...
    sources = "".join(path.read_text(encoding="utf-8") for path in
                      list((ROOT / "src").glob("*.cpp")) + list((ROOT / "include").glob("*.h")))
    text = "".join(re.findall(r'"(?:\\.|[^"\\])*"', sources))
    ...
    for line in gzip.decompress(SOURCE.read_bytes()).decode("ascii").splitlines():
        ...
        if point in needed:
            ...
            if point >= 128 and len(data) == 16:
                data = b"".join(bytes((byte, 0)) for byte in data)
    ...
    lines = ["// Generated by scripts/generate-font.py; do not edit.",
             ...
             "#pragma once", "#include <stdint.h>", "namespace codex_usage_font {",
    ...
    lines += ["};", "}  // namespace codex_usage_font", ""]
