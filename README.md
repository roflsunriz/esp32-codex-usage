...
6. ESP32はAPから切断された後も認証完了をポーリングします。承認後はトークンをNVSへ保存し、約5分ごとに使用量を更新します。手動の「更新」操作とWi-Fi再接続時はすぐに取得します。
...
- **使用量**: 5時間制限と週間制限のプログレスバー、パーセンテージ、リセット日時と残り時間、更新状態。残り時間は基本「あとX日Y時間」、24時間未満は「あとX時間Y分」で表示します。状態表示には次の取得までの残り秒数（例: 更新済み あと42秒）も表示します。
- **設定**: 自動消灯を0〜59分のスライダーと0〜24時間のスライダー、取得間隔を60〜600秒（60秒刻み）のスライダーで調整。タブ内はスクロールバーや内容ドラッグで移動できます。0分0時間は自動消灯オフ（常時点灯）と同じです。
